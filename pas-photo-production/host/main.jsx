/* =====================================================================
 * Pas Foto Print v2.3.0 - host/main.jsx
 * (user-provided version with noBorder support, updated createDocument,
 *  directional duplicateSlot, and reselect fix)
 *
 * FIX (Generate ulang tidak menghilangkan edit manual yang sudah ada):
 *  - Sebelumnya api.generate() dengan opsi "Ganti hasil sebelumnya" aktif
 *    (default) selalu menghapus SELURUH layer hasil generate lama lalu
 *    membangun ulang dari nol berdasarkan data panel saja. Akibatnya edit
 *    manual yang dilakukan langsung di artboard (ganti background per-slot,
 *    ganti foto per-slot, crop, flip, dan slot hasil "Duplicate") selalu
 *    ikut hilang setiap kali generate ditekan ulang.
 *  - Sekarang, SEBELUM layer lama dihapus, api.generate() memanggil
 *    collectPreservedState() untuk mengambil edit-edit tsb dari slot yang
 *    sudah ada (dikenali lewat slotId, yang stabil lintas generate selama
 *    baris pesanan ukurannya tidak dihapus dari panel - lihat createSlot()
 *    untuk detail apa saja yang di-restore). Slot hasil "Duplicate" (di
 *    luar struktur baris pesanan) dipindah ke layer tersendiri, tidak ikut
 *    dihapus maupun disusun ulang.
 *  - Batasan yang disengaja: rotasi manual ("Putar 90") pada satu slot
 *    TIDAK ikut dipertahankan otomatis, karena itu mengubah orientasi fisik
 *    slot yang dipakai algoritma penyusunan (layout-engine) untuk menyusun
 *    ulang lembar secara rapi - menimpanya paksa dengan rotasi lama bisa
 *    membuat foto/bingkai tidak sinkron dengan hasil susun ulang yang baru.
 *    Slot yang pernah diputar manual sebaiknya dicek ulang setelah generate.
 * ===================================================================== */
...
(function (api) {
  ...
  /* Cari rectangle background yang sudah ada di dalam clipGroup (jika ada).
     Di dalam clipGroup hanya ada 2 kemungkinan PathItem: clip mask (clipping=true)
     dan background rect (clipping=false, filled). PlacedItem/RasterItem foto diabaikan. */
  ...
  /* override (opsional): hasil edit manual pada slot dengan slotId yang sama
     ...
     Kalau ada, background/foto sumber/crop/flip dari override dipakai
     menggantikan nilai default baris pesanan (slot.*) - TIDAK termasuk
     rotasi (slot.rotation tetap dipakai apa adanya dari hasil packing
     terbaru, karena rotasi itu bagian dari keputusan susun-ulang layout,
     bukan atribut visual independen). */
  function createSlot(doc, parent, slot, origin, mode, noBorder, fitMode, override) {
    ...
    var sourcePath = (ov && ov.sourcePath) ? ov.sourcePath : slot.sourcePath;
    var sourceId = (ov && ov.sourceId) ? ov.sourceId : slot.sourceId;
    ...
    var bgHex = (ov && typeof ov.backgroundColor === "string") ? ov.backgroundColor : ((typeof slot.backgroundColor === "string") ? slot.backgroundColor : "");
    var cropVal = (ov && ov.crop) ? ov.crop : (slot.crop || { x: 0, y: 0, scale: 100 });
    var flipH = ov ? !!ov.flipH : false;
    var flipV = ov ? !!ov.flipV : false;
    var meta = { pfpm: true, version: 2, jobId: origin.jobId, sheet: origin.sheetIndex, slotId: slot.slotId, sizeId: slot.sizeId, label: slot.label, sourceId: sourceId, sourcePath: sourcePath, crop: cropVal, rotation: num(slot.rotation, 0), flipH: flipH, flipV: flipV, physicalWidth: slot.physicalWidth, physicalHeight: slot.physicalHeight, offsetBorder: num(slot.offsetBorder !== undefined ? slot.offsetBorder : slot.borderWidth, 1.5), mode: mode, fitMode: (fitMode === "fit") ? "fit" : "fill", backgroundColor: bgHex, frame: { left: left, top: top, w: fw, h: fh } };
    meta.frame = { left: left, top: top, w: fw, h: fh };
    var clipGroup = parent.groupItems.add(); clipGroup.name = baseName;
    var clip = clipGroup.pathItems.rectangle(top, left, fw, fh); clip.name = "FRAME " + slot.physicalWidth + "x" + slot.physicalHeight + "mm"; clip.stroked = false; clip.filled = false;
    var bgRect = addBackgroundRect(clipGroup, top, left, fw, fh, bgHex, baseName);
  ...
  /* collectPreservedState: dipanggil SEBELUM clearGenerated() saat generate
     ...
     - duplicateGroups = daftar slot hasil "Duplicate" beserta border-nya;
       ini dipindah ke layer terpisah supaya tidak ikut terhapus, TAPI juga
    ...
    try {
      ...
      for (i = 0; i < layout.sheets.length; i++) {
        ...
        for (j = 0; j < sheet.slots.length; j++) {
          ...
          if (ov) restoredCount++;
          createSlot(doc, parent, sheet.slots[j], origin, mode, !!opts.noBorder, opts.fitMode, ov);
          totalSlots++;
        }
