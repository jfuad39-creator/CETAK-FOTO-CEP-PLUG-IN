 ...
 *  - buildJob() sekarang menyertakan field `sizeId` MENTAH (mis. "4x6",
 *    "3x4") secara terpisah dari `id` komposit yang dipakai layout-engine
 *    untuk membuat slotId unik. Sebelumnya field ini tidak dikirim sama
 *    sekali, sehingga layout-engine terpaksa memakai id komposit
 *    ("4x6|ab12") sebagai sizeId tiap slot. Akibatnya dropdown "Terapkan
 *    ke Ukuran Ini" pada panel (yang berisi sizeId mentah seperti "4x6")
 *    tidak pernah cocok dengan sizeId yang tersimpan di slot hasil
 *    generate, sehingga fitur "ganti background ke semua slot ukuran
 *    tertentu" selalu gagal ("Tidak ada slot ukuran ... di dokumen ini").
 *    Lihat juga perbaikan terkait di client/js/layout-engine.js.
 * ===================================================================== */
(function (root) {
  "use strict";
  ...
  function addPhoto(name, path, thumb) {
    var p = { id: uid("photo"), name: name, path: path, thumb: thumb || "", color: nextColor(), bg: "" };
    state.photos.push(p);
    save();
    return p;
  }
