/* =====================================================================
 * client/js/app.js - controller panel Pas Foto Production Manager v2.4.1
 *
 * FIX v2.4.1:
 *  - Ganti Background pada slot yang sudah di-generate sekarang bekerja:
 *    1. "Terapkan ke Slot Terpilih" - menerapkan ke slot yang diseleksi
 *    2. "Terapkan ke Ukuran Ini" - menerapkan ke semua slot dengan ukuran tertentu
 *  - Dropdown ukuran (bgSizeTarget) diisi dari slot yang ada di dokumen Illustrator
 *    (bukan dari order list panel) melalui API getGeneratedSizes baru.
 *  - Preview scale calculation: mengurangi extra margin agar sheet tidak terpotong.
 *  - renderSizePresets() selalu dipanggil saat init.
 * ===================================================================== */
(function () {
  "use strict";
  ...
  /* =========================================================================
   * FIX: Dropdown "Terapkan ke Ukuran Ini" sekarang diisi dari slot yang
   * ada di dokumen Illustrator (hasil generate), bukan dari Order List panel.
   * Ini memungkinkan ganti background pada slot yang sudah di-generate
   * meskipun panel sudah di-reset atau order diubah.
   * ========================================================================= */
  function refreshBgSizeTarget() {
    if (!B.isCEP) {
      renderBgSizeTargetFromPanel();
      return;
    }
    
    B.call("getGeneratedSizes", {}, function (res) {
      var sel = $("#bgSizeTarget");
      if (!sel) return;
      
      if (!res.ok || !res.data || !res.data.sizes || !res.data.sizes.length) {
        // Fallback ke panel order list jika tidak ada slot di dokumen
        renderBgSizeTargetFromPanel();
        return;
      }
      ...
      sel.innerHTML = sizes.map(function (s) {
        return '<option value="' + esc(s.sizeId) + '">' + esc(s.label) + ' (' + s.physicalWidth + '\u00d7' + s.physicalHeight + ')</option>';
      }).join("");
    });
  }

  /* Fallback: isi dropdown dari order list panel (perilaku lama) */
  function renderBgSizeTargetFromPanel() {
    var sel = $("#bgSizeTarget");
    if (!sel) return;
    var st = S.get();
    var seen = {}, opts = [];
    ...
    sel.innerHTML = opts.length
      ? opts.map(function (it) {
          return '<option value="' + esc(it.sizeId) + '">' + esc(it.label) + " (" + it.width + "\u00d7" + it.height + ")</option>";
        }).join("")
  ...
  $("#btnGenerate").onclick = function () {
    ...
    B.call("generate", {
      ...
      // Refresh dropdown ukuran setelah generate
      if (res.ok) {
        setTimeout(refreshBgSizeTarget, 500);
      }
    });
  };
  ...
  $("#btnDelete").onclick = function () {
    B.call("deleteSlot", {}, function (r) { toast(r.message, r.ok ? "ok" : "err"); refreshSelection(true); refreshBgSizeTarget(); });
  };
  ...
  /* =========================================================================
   * FIX: Tombol "Terapkan ke Slot Terpilih" sekarang memanggil setBackground
   * yang sudah diperbaiki di host/main.jsx
   * ========================================================================= */
  $("#btnBgApplySel").onclick = function () {
    if (!B.isCEP) return toast("Hanya tersedia di dalam Illustrator.", "err");
    B.call("setBackground", { hex: pendingBg }, function (r) { 
  ...
  /* =========================================================================
   * FIX: Tombol "Terapkan ke Ukuran Ini" sekarang memanggil setBackgroundBySize
   * yang sudah diperbaiki di host/main.jsx untuk mengiterasi semua layer
   * ========================================================================= */
  $("#btnBgApplySize").onclick = function () {
    if (!B.isCEP) return toast("Hanya tersedia di dalam Illustrator.", "err");
    var sizeId = $("#bgSizeTarget").value;
    if (!sizeId) return toast("Tidak ada ukuran untuk diterapkan.", "err");
    B.call("setBackgroundBySize", { sizeId: sizeId, hex: pendingBg }, function (r) { 
      toast(r.message, r.ok ? "ok" : "err"); 
    });
  };
  ...
  function init() {
    ...
    info.title = connected
      ? "Terhubung ke " + (B.host.appName || "Illustrator") + " " + (B.host.appVersion || "") + " \u00b7 v2.4.1"
      : "Mode browser (preview saja) \u2014 buka di dalam Illustrator untuk fitur penuh. v2.4.1";
    $$(".logo-dot, .status-dot").forEach(function (d) { d.classList.toggle("off", !connected); });

    if (B.isCEP) {
      B.call("ping", {}, function () {});
      // Refresh dropdown ukuran dari dokumen yang sudah ada
      refreshBgSizeTarget();
