 ...
 *
 * Strategi utama untuk ukuran produksi standar 4x6, 3x4, 2x3:
 * template presisi referensi untuk A4 dan A5, gap 0mm.
 *
 *  A4 portrait (margin default 5mm, gap 0):
 *     4x6 portrait  : 4 kolom x 1 baris  = 160x60mm
 *     3x4 rotated   : 4 kolom x 1 baris  = 160x30mm
 *     2x3 portrait  : 2 kolom x 3 baris  =  40x90mm
 *     Total blok persis 200x90mm, memenuhi lebar area cetak A4.
 *
 *  A5 portrait (margin default 5mm, gap 0):
 *     4x6 rotated   : 2 kolom x 2 baris  = 120x80mm
 *     3x4 portrait  : 4 kolom x 1 baris  = 120x40mm
 *     2x3 portrait  : 6 kolom x 1 baris  = 120x30mm
 *     Total blok persis 120x150mm, dipusatkan horizontal di A5.
 *
 *  Offset border TIDAK masuk hitungan layout dan TIDAK mengurangi ukuran
 *  foto aktual. Ukuran clipping mask tetap 40x60, 30x40, 20x30mm; border
 *  hitam 0.3pt digambar oleh host/main.jsx keluar dari mask sesuai offset.
 *
 *  Untuk ukuran/jumlah lain, engine fallback ke horizontal band packing:
 *  isi satu baris sampai mentok margin, baru turun. Satu band hanya berisi
 *  item dengan tinggi orientasi sama agar tidak muncul lubang kosong di bawah
 *  foto yang lebih pendek.
 *
 * Output: daftar sheet berisi slot absolut (mm) siap dikirim ke
 * host/main.jsx.
 * ===================================================================== */
(function (root) {
  "use strict";
  ...
  function build(job, landscape) {
    var base = MEDIA_SIZES[job.media.type] || MEDIA_SIZES.A4;
    ...
    var offsetBorder = Math.max(0, clampNum(job.options && (job.options.offsetBorder !== undefined ? job.options.offsetBorder : job.options.borderWidth), 1.5));
    ...
    for (var i = 0; i < (job.items || []).length; i++) {
      ...
      items.push({ id: it.id, label: it.label || it.id, w: w, h: h, qty: qty, srcId: it.sourceId, srcPath: it.sourcePath || "", seq: 1 });
    }
    ...
    return { media: { type: job.media.type, width: pw, height: ph, margin: margin, gap: gap, landscape: !!landscape }, sheets: sheets, totalSlots: totalRequested, placed: placed, unplaced: totalRequested - placed, efficiency: efficiency, options: { offsetBorder: offsetBorder, rotateMode: rotateMode, cutGuide: !!(job.options && job.options.cutGuide), grouping: (job.options && job.options.grouping) || "flat" } };
  }
